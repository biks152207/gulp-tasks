(function () {
    app.directive('stickyHeader', ['$ionicPosition', '$compile','$rootScope', function ($ionicPosition, $compile,$rootScope) {
        return {
            restrict: 'A',
            require: '^$ionicScroll',
            link: function ($scope, $element, $attr, $ionicScroll) {
                ...
                var clone;
                var headerHeight = ionic.Platform.isIOS() ? 90 : (ionic.Platform.isAndroid()  ? 70: 5 );
                ...
                var lastActive;
                var updateSticky = ionic.throttle(function(force) {
                    ...
                    for (var i = 0; i < dividers.length; ++i) { // can be changed to binary search
                        if ($ionicPosition.offset(dividers[i]).top - dividers[i].prop('offsetHeight') < headerHeight) { // this equals to jquery outerHeight
                            if (i === dividers.length-1 || $ionicPosition.offset(dividers[i+1]).top -
                                (dividers[i].prop('offsetHeight') + dividers[i+1].prop('offsetHeight')) >headerHeight) {
                                active = dividers[i][0];
                                break;
                            }
                        }
                ...
                $rootScope.$on('update-sticky',function(){
                   updateSticky(true);
                });
